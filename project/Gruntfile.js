	...
	grunt.initConfig({
			...
			development: {
				constants: {
					ENV: {
						...
						//
						// Where the settings values will come from.
						// Available option:
						//   LOCAL    : hardcoded withing the app code
						//   FIREBASE : values are retrieved from Firebase
						//   PARSE : values are retrieved from Parse.com
						settingsSource: 'FIREBASE'
					}
				}
			},
			production: {
		...
		wiredep: {
			app: {
				...
				ignorePath: /\.\.\//,
				exclude: [/jquery/, /bower_components\/bootstrap/, 'app/bower_components/ionic/release/css/ionic.css']
			}
		},
		sass: {                                   // Task
			dist: {                               // Target
